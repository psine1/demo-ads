...
//  const status = document.getElementById('status');
  ...
  // ===== Grid/Layout =====
  let cell=24, offX=0, offY=0, pad=12;
  ...
  function drawMaze(){
    mazeG.innerHTML="";
    const t=Math.round(cell*0.5), eps=0.5, half=cell/2;
    for (let r=0;r<rows;r++){
      for (let c=0;c<cols;c++){
        if (MAZE[r][c] !== '#') continue;
        const cx=offX+c*cell+half, cy=offY+r*cell+half;
        rect(cx - t/2, cy - t/2, t, t);
        if (c+1<cols && MAZE[r][c+1]==='#') rect(cx, cy - t/2, half+eps, t);
        if (c-1>=0   && MAZE[r][c-1]==='#') rect(cx - half - eps, cy - t/2, half+eps, t);
        if (r-1>=0   && MAZE[r-1][c]==='#') rect(cx - t/2, cy - half - eps, t, half+eps);
        if (r+1<rows && MAZE[r+1][c]==='#') rect(cx - t/2, cy, t, half+eps);
      }
    }
    function rect(x,y,w,h){
      ...
      el.setAttribute("fill","var(--wall)"); el.setAttribute("filter","url(#wallInset)");

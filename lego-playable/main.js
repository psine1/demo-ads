...
var multitimeline = gsap.timeline();
    ...
    const tl_video = gsap.timeline({paused: false});
      tl_video.timeScale(1);  
      tl_video
      .from('.skip, .controls', 1, {autoAlpha: 0, ease: "powe3.out" }, "<+1")      




    const tl_game = gsap.timeline({paused: true});
      tl_game.timeScale(1);  
      tl_game
...
// ===== Config =====
const VIDEO_SRC  = 'video.mp4';   // ← tu mp4 (H.264/AAC)
const POSTER_SRC = '';            // opcional: 'poster.jpg'
const START_MUTED = true;         // debe iniciar muteado
const AUTOPLAY_TRY = true;        // intentar autoplay muted
...
function onVideoEnded(){
  ...
  // opcional: actualizar UI a "Replay"
  if (btnPlayPause) {
    btnPlayPause.setAttribute('aria-label', 'Replay');
  }
  updateUI();
}
...
function skipToEnd(){
  // Si la duración aún no está disponible, esperamos el metadata
  const doSkip = () => {
    const d = video.duration || 0;
    // Pausamos y saltamos “al final”
    video.pause();
    try {
      // movernos apenas antes del final evita edge cases
      const EPS = 0.05; // 50 ms
      video.currentTime = d > EPS ? (d - EPS) : d;
    } catch(e){ /* ignore */ }

    // Algunos navegadores no disparan 'ended' si el video está pausado.
    // Reutilizamos la misma lógica manualmente:
    onVideoEnded();

    // Opcional: contadores de RM
    // if (window.Enabler) Enabler.counter('Skip');
  };

  // ¿Tenemos duración?
  if (!isFinite(video.duration) || !video.duration) {
    video.addEventListener('loadedmetadata', doSkip, { once:true });
    // si querés forzar el load por si no arrancó aún:
    if (video.readyState === 0) video.load();
  } else {
    doSkip();
  }

Enabler.exit('Skip video');    
...
video.addEventListener('ended', () => {
  tl_game.play(0);
  // si querés cambiar el botón a "Replay":
  btnPlayPause.setAttribute('aria-label','Replay');

  // si usás íconos, actualizalos acá también
});




// ===== Helpers (CM360/Studio) =====
function url(file){
  try {
    return (window.Enabler && Enabler.getUrl)
      ? Enabler.getUrl(file)
      : file;
  } catch(e){ return file; }
}
...
// ===== Init gating (Studio visible) =====
(function init(){
  if (window.Enabler && window.studio){
    const onInit = () => {
      if (Enabler.isVisible()) start();
      else Enabler.addEventListener(studio.events.StudioEvent.VISIBLE, start);
    };
    if (Enabler.isInitialized()) onInit();
    else Enabler.addEventListener(studio.events.StudioEvent.INIT, onInit);
  } else {
    // fuera de Studio
...
// ===== Lógica principal =====
function start(){
  // Atributos imprescindibles para mobile
  ...
  if (POSTER_SRC) video.poster = url(POSTER_SRC);

  // Fuente cargada SOLO desde JS (sin <source> en HTML)
  video.src = url(VIDEO_SRC);
  video.load();

  // Intento de autoplay muted (al tener datos)
  const tryAutoplayMuted = () => {
    if (!AUTOPLAY_TRY) return;
    // asegurar estado muteado previo al play()
    if (START_MUTED){
      video.muted = true;
      video.defaultMuted = true;
      video.volume = 0;
    ...
    if (p && p.catch){
      p.catch(() => {
        // Si falla, quedamos a la espera del gesto (click en Play)
        updateUI();
      });
    }
  };

  if (video.readyState >= 2) tryAutoplayMuted();
  else {
    video.addEventListener('loadeddata', tryAutoplayMuted, { once:true });
    video.addEventListener('canplay',     tryAutoplayMuted, { once:true });
  }

  // Eventos UI
  btnPlayPause.addEventListener('click', onTogglePlayPause);
  btnMute.addEventListener('click', onToggleMute);

  // Sync UI con estado real del <video>
  video.addEventListener('play',  updateUI);
  video.addEventListener('pause', updateUI);
  video.addEventListener('ended', () => { /* no loop por defecto */ updateUI(); });
  video.addEventListener('volumechange', updateUI);

  // Opcional: pausa si la pestaña se oculta (friendly con viewability)
...
function onTogglePlayPause(){
  if (video.paused || video.ended){
    video.play().catch(()=>{ /* si estuviera bloqueado, el mismo click ya es gesto */ });
    Enabler.exit('play video');
  } else {
    video.pause();
    Enabler.exit('pause video');   
  }
}

function onToggleMute(){
  const goingToUnmute = video.muted || video.volume === 0;
  if (goingToUnmute){
    // Este click es el gesto válido para habilitar audio
    video.muted = false;
    video.defaultMuted = false;
    video.removeAttribute('muted');
    video.volume = 1;
    Enabler.exit('unmute video');
    // Opcional: Enabler.counter('Unmute');
  } else {
    video.muted = true;
    video.defaultMuted = true;
    video.setAttribute('muted','');
    video.volume = 0;
    Enabler.exit('mute video');
    // Opcional: Enabler.counter('Mute');
  }
  updateUI();
}
...
// Rutas de íconos (usa tu carpeta/formatos). Pueden ser .svg o .png
const ASSETS = {
  play:   'icons/play.svg',
  pause:  'icons/pause.svg',
  mute:   'icons/unmute.svg',    // estado: con sonido (botón muestra MUTE)
  unmute: 'icons/mute.svg'   // estado: silenciado (botón muestra UNMUTE)
};

// Helper para aplicar Enabler.getUrl() a imágenes también
function icon(file){ return url(file); }

// (Opcional) Preload para evitar parpadeos
['play','pause','mute','unmute'].forEach(k => { const i = new Image(); i.src = icon(ASSETS[k]); });

function updateUI(){
  // Play/Pause
  const isPlaying = !video.paused && !video.ended;
  btnPlayPause.setAttribute('aria-label', isPlaying ? 'Pause' : 'Play');
  icoPlayPause.src = icon(isPlaying ? ASSETS.pause : ASSETS.play);

  // Mute/Unmute
  const isMuted = video.muted || video.volume === 0;
  btnMute.setAttribute('aria-label', isMuted ? 'Unmute' : 'Mute');
  icoMute.src = icon(isMuted ? ASSETS.unmute : ASSETS.mute);
}

...
function imagesLoaded() {
    ...
    function wireInteractions(){
      document.getElementById('ctaPortrait')?.addEventListener('click', e=>{
        e.preventDefault();
        // TODO: Tracking portrait (ej. Enabler.counter('Click_Portrait'))
        console.log('CTA Portrait');
      });
      document.getElementById('ctaLandscape')?.addEventListener('click', e=>{
        e.preventDefault();
        // TODO: Tracking landscape (ej. Enabler.counter('Click_Landscape'))
...
var multitimeline = gsap.timeline();
    ...
    gsap.set(".button-players_P", {scale: 1.2})
    gsap.set(".wrapCharacters_P", {scale: 0.85})
    ...
    const tl_game = gsap.timeline({paused: false});
      ...
      .from('.bg, .bg_P', 1.2, {scale: 1.8, rotation: 0.05, transformOrigin: "50% 50%", ease: "power3.out" }, "<")      
      .from('.sun, .sun_P', 1.5, {scale: 2.5, rotation: 0.05, transformOrigin: "50% 50%", ease: "power3.out" }, "<")              
      .from('[id*="car1"]', 0.5, {scale: 0.3, rotation: 0.05, x: 400, y: -80, transformOrigin: "50% 50%", ease: "power3.out" }, "<")      
      .from('[id*="man1"]', 0.65, {y: -100, rotation: 0.05,  transformOrigin: "50% 50%", ease: "power3.out" }, "<+0.25")      
      .from('[id*="man1"]', 0.25, {autoAlpha: 0, ease: "power3.out" }, "<")    
      .from('[id*="productName1"]', 0.65, {y: 160, ease: "power3.out" }, "<")    
      .from('[id*="productName1"]', 0.25, {autoAlpha: 0, ease: "power3.out" }, "<")          
      .from('[id*="boxToy1"]', 0.65, {x: -700, ease: "power3.out" }, "<+0.15")    
      .from('[class*="selectPlayer-"]', 0.35, {scale: 0, stagger: {  from: "random", amount: 0.3 }, ease: "back.out(1.7)" }, "<")    
      .from('.text-gamme, .text-gamme_P', 0.65, {y: 20, autoAlpha: 0, ease: "power3.out" }, "<+0.15")    
      .from('#cta-game, #cta-game_P', 0.65, {scale: 0, ease: "back.out(1.7)" }, "<+0.35")    

    const tl_endFrame = gsap.timeline({paused: true});
      tl_endFrame.timeScale(1);  
      tl_endFrame
      .to('#clickTag', 0.01, {autoAlpha: 1, ease: "powe3.out" }, "<")      
      .to('.text-game, .text-game_P', 0.35, {y: 20, autoAlpha: 0, ease: "power3.out" }, "<+0.15")    
      .to('[class*="selectPlayer-"]', 0.35, {scale: 0, stagger: {  from: "random", amount: 0.3 }, ease: "power3.out" }, "<")    
      .to('#cta-game, #cta-game_P', 0.35, {scale: 0, ease: "power3.out" }, "<+0.35")    
      ...
      .from('.cta-endframe, .cta-endframe_P', 0.65, {scale: 0, ease: "back.out(1.7)" }, "<+0.5")    

    let ctaGame = document.getElementById('cta-game');
    ctaGame.addEventListener('mouseup', function(event) {             
        
        tl_endFrame.play();
        Enabler.counter('get started');

    })

    let ctaGame_P = document.getElementById('cta-game_P');
    ctaGame_P.addEventListener('mouseup', function(event) {             
